...
class Analysis:
    ...
    def scoring_kan(self, ms):
        ...
        for tf in ms.meishiki["twelve_fortune"]:
            if tf == -1:
                continue
            ms.analysis["kan_score"] += self.twelve_fortune_score[tf] / 2
    ...
    tsuhen_character = [
        ...
        [
            ...
            "物事をつきつめていくタイプで、束縛されることが嫌いで組織には向きません。技芸や芸術方向を伸ばせば見通しが明るくなりそうです。",  # 食神 x 偏官
        ...
        [
            "なにかにつけて我を通そうとするため、身近な人とトラブルになりやすいのが心配です。行動力があり、目標に向かって努力できるので、少し我を抑えれば活躍の幅は徐々に広がっていきます。趣味を実益化すると見通しが明るいでしょう。",  # 偏財 x 比肩
        ...
        [
            ...
            "一人の中に正反対の性格が同居するタイプなため、迷いが多くなりがちです。１つのことに集中する気概を持てば、見通しが明るいでしょう。",  # 正官 x 偏官
            "律儀でしっかりした人格の持ち主です。情よりも義を優先するため、ときに非情な判断を下すことになるでしょう。調子に乗りやすいところがあるので、余計な敵を作ってしまうおそれがあります。日頃の言動に注意が必要です。",  # 正官 x 正官
        ...
        [
            ...
            "やや保守的なタイプです。一風変わった分野に興味が向かい、考え方が偏りがちです。組織や集団生活ではワガママに注意が必要です。自分のこだわりを活かせる仕事を選べば見通しが明るいでしょう。",  # 偏印 x 正官
            "ユニークな発想で成功を収める可能性が高いタイプです。しかし、気分のムラや人の好き嫌いが激しく、つい自分勝手なやり方になってしまうので、ついて来られる人は少なくなります。少し我を抑える努力が必要になるでしょう。",  # 偏印 x 偏印
